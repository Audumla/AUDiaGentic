# Application Capabilities architecture lock

Status: **normative lock target** for all production Rust code from `audiagentic-core` through reusable Application Capabilities. Provider, harness, AgentWork, Context, gateway, protocol, UI and distributed-runtime semantics remain above this line.

This document turns the current Rust shape into an explicit architecture contract. The goal is not to make the platform generic; it is to make the existing small layers hard to misuse.
...
| errors | stable public error definition vocabulary only | GREEN after boundary-lock proof |
...
| config | typed extraction + ordered in-memory source resolution + config revision; no source I/O | GREEN after boundary-lock proof |
...
| events | ordered domain events, bounded retention/cursors, explicit `EventPolicy`; no broker/telemetry | GREEN after boundary-lock proof |
| workflow | deterministic state transition/effects/snapshots; no runtime/I/O | GREEN after boundary-lock proof |
| time | deterministic caller-supplied time/deadline/timer-set semantics | GREEN after boundary-lock proof |
| managed-config | single-writer observe/plan/apply over `FileHost` + reconcile | GREEN after boundary-lock proof |
...
`scripts/rust-foundation-smoke.sh` is part of this lock. In addition to formatting, Clippy, tests, executable proofs and dependency direction, it must enforce:
...
The next architecture work should not move above this lock line until the Rust CI proves these rules on Ubuntu, macOS and Windows.
