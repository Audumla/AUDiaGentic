...
Expose model operations as thin bindings over the generalized core:
...
- `qualify_provider_model(provider_id, source_id, model_id, required_capabilities)`
- `list_provider_model_qualification(provider_id, source_id=None, model_id=None)`

Use:

- `reconcile_fragments` + `FragmentStore` for structured entries;
- one parameterized registry instance at `.audiagentic/runtime/providers/managed-model-endpoints.json`;
- a separate qualification cache containing no secrets;
- a model-specific recipe kind;
- `NoAutomationRecipe` only for manual/unsupported/blocked cases;
- a dedicated launch-env contribution recipe;
- a sibling model sync call inside `reconcile.py::reconcile_provider`.

### Source-management tools

All mutations validate and write `.audiagentic/config/model-sources.yaml`, support `dry_run`, and optionally reconcile:
...
- `model_source_refresh_catalog(source_id)`
- `model_source_qualify(source_id, provider_ids=None, model_ids=None)`

### Projection tools

- `sync_provider_models(provider_id, dry_run=False, managed_ids=None)`
- `list_provider_models_config(provider_id)`
- `reload_provider_models(provider_id)`
