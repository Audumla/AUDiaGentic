...
ACP support also does not by itself establish an observability level. The prompt
response can provide an O2 turn boundary when its correlation and semantics are
validated, while O3 model/tool phase visibility depends on the concrete agent and
adapter events. AUDiaGentic normalizes those events as evidence under the
[Harness Observability & Lifecycle Capability Model](../harnesses/harness-observability-lifecycle.md);
raw ACP event names never become scheduler state directly.
