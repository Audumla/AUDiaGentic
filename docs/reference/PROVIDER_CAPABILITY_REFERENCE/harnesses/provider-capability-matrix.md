...
This table covers provider/budget telemetry. Harness lifecycle observability is a
separate capability axis and uses O0–O4 from
[Harness Observability & Lifecycle Capability Model](harness-observability-lifecycle.md).
...
The telemetry dimension informs budget, quota, context, and provider-selection
scheduling. Lifecycle dispatch, persistent reuse, and resource-slot release are
gated independently by the harness observability level. Here's what each telemetry
tier can do:
