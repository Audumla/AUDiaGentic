...
[CAPABILITY_OPERATION_SCHEMA.md](CAPABILITY_OPERATION_SCHEMA.md) and descriptor
operational declarations to stable `capability_id` values for `capability_facts`.

Each provider declares `capability_facts` entries using these IDs. A fact asserts
that a provider *has* or *does not have* a specific capability, with evidence.

## Capability ID taxonomy

IDs use the `<domain>-<operation>` pattern. Domain prefixes:
...
For each provider, the capability IDs it possesses (field present and non-empty) vs. does not possess (field absent/empty/None). This is the source of truth for populating `capability_facts`.
...
4. **Omitted** — for capabilities the provider clearly does not have (field is None/empty and no evidence suggests otherwise). Do not create negative facts unless there's a specific gap or concern.
...
| Evidence docs | `docs/reference/model-source-evidence/<id>.md` | Model-related capabilities (catalog, connectors, vendor injection) |
| Capability matrix | `docs/reference/PROVIDER_MODEL_ENDPOINT_CAPABILITIES.md` | Cross-reference for model connector support, projection modes |
...
- Evidence doc: `docs/reference/model-source-evidence/<id>.md#<section>`
- Capability matrix: `docs/reference/PROVIDER_MODEL_ENDPOINT_CAPABILITIES.md#<provider>`
