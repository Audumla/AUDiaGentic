# PKT-JOB-006 — Release script integration from jobs
...
Connect jobs to release change capture and summary generation without duplicating release logic.
...
```python
def emit_job_change(job, change_payload):
    event = build_change_event_from_job(job, change_payload)
    record_change_event(event)
    sync_current_release_ledger(job.ctx)
    regenerate_current_release(job.ctx)
```
...
- job/release bridge fixtures

## Tests to add

- jobs update tracked release docs only via release scripts
...
- job-to-release bridge implemented
