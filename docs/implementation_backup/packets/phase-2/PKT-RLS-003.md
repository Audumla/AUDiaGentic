# PKT-RLS-003 — Regenerate current release summary
...
Generate `docs/releases/CURRENT_RELEASE.md` from the current ledger only.
...
```python
def regenerate_current_release(ctx):
    events = load_current_release_ledger(ctx)
    grouped = group_events(events, by=["change-class","module"])
    markdown = render_current_release(grouped)
    atomic_write_text(ctx.current_release_md, markdown)
```
...
- CURRENT_RELEASE fixtures

## Tests to add

- summary regeneration is deterministic and stable-order
...
- CURRENT_RELEASE.md generation implemented
