# PKT-RLS-004 — Generate audit and check-in summaries
...
Generate the tracked latest audit/check-in docs from current ledger plus job-independent inputs.
...
- `tests/integration/release/test_audit_summary.py`
...
```python
def generate_audit_and_checkin(ctx):
    events = load_current_release_ledger(ctx)
    audit = build_audit_summary(events, approvals=load_latest_approvals(ctx))
    checkin = build_checkin_summary(events, audit)
    atomic_write_text(ctx.audit_summary_md, audit)
    atomic_write_text(ctx.checkin_md, checkin)
```
...
- audit/checkin fixtures

## Tests to add

- CHECKIN.md and AUDIT_SUMMARY.md are generated by scripts only
...
- audit + check-in docs generation implemented
