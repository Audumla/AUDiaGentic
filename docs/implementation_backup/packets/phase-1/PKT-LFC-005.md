...
Implement cutover from legacy projects into the new `.audiagentic/` layout.
...
- `tests/e2e/lifecycle/test_cutover.py`
...
```python
def apply_cutover(ctx):
    assert detect_installed_state(ctx.project_root) == "legacy-only"
    migrate_docs(ctx)
    migrate_changelog(ctx)
    install_new_baseline(ctx)
    cleanup_legacy_runtime(ctx)
    return success_result(cutover=True)
```
...
- cutover produces reports and preserves legacy workflow by rename
...
- legacy cutover plan/apply/validate works
