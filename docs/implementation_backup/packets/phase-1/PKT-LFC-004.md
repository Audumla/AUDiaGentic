# PKT-LFC-004 — Update dispatcher and version module selection
...
Implement version-aware update dispatch without doing the update logic itself.
...
- `tests/unit/lifecycle/test_update_dispatch.py`
...
```python
def select_update_module(installed_version, target_version):
    if installed_version.major == target_version.major:
        return import_module(f"update_v{installed_version.major}")
...
- version manifest fixtures

## Tests to add

- dispatcher selects correct module for same-major/same-minor updates
...
- update dispatch contract implemented
