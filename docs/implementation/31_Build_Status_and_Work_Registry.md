...
| PKT-PRV-034 | Gemini prompt-trigger launch integration | IN_PROGRESS | Codex | workspace | needs PKT-PRV-031 VERIFIED + PKT-PRV-006 VERIFIED | 03, 29, 40, packet | 2026-03-30 | BLOCKING: .gemini/commands/ directory and per-tag command templates do not exist; BeforeAgent hook config absent; no smoke tests; packet updated with explicit missing-implementation section |
| PKT-PRV-035 | GitHub Copilot prompt-trigger launch integration | IN_PROGRESS | Codex | workspace | needs PKT-PRV-031 VERIFIED + PKT-PRV-007 VERIFIED | 03, 29, 40, packet | 2026-03-30 | BLOCKING: all .github/prompts/*.prompt.md and .github/agents/*.agent.md files are stubs; implement/audit/check-in-prep agents missing entirely; no smoke tests; packet updated with explicit missing-implementation section |
...
| PKT-PRV-037 | Cline prompt-trigger launch integration | IN_PROGRESS | Codex | workspace | needs PKT-PRV-031 READY_FOR_REVIEW + PKT-PRV-009 VERIFIED | 03, 29, 40, packet | 2026-03-30 | BLOCKING: .clinerules/skills/ directory and per-tag skill files do not exist; no hook config; no smoke tests; packet updated with explicit missing-implementation section |
...
| Packet | Title | Status | Owner | Scope | Dependencies | Docs | Updated | Notes |
|---|---|---|---|---|---|---|---|---|
| PKT-PRV-048 | Shared provider live-stream capture contract + harness | READY_TO_START | Codex | workspace | needs PKT-PRV-031 VERIFIED + PKT-PRV-022 VERIFIED | 03, 34, 45, packet | 2026-04-02 | REQUIRES generic StreamSink protocol + ConsoleSink/RawLogSink/NormalizedEventSink built-ins + stdout_sinks/stderr_sinks list params + per-sink error isolation; replaces hardcoded tee_console/log_path; spec 34 and impl doc 45 are locked |
| PKT-PRV-049 | Codex live-stream capture integration | READY_TO_START | Codex | workspace | needs PKT-PRV-048 READY_TO_START or better | 03, 34, 45, packet | 2026-04-02 | registers ConsoleSink+RawLogSink+NormalizedEventSink for Codex runs; adds CodexEventExtractor sink in adapters/codex.py; shared harness must not be modified |
| PKT-PRV-050 | Cline live-stream capture integration | READY_TO_START | Codex | workspace | needs PKT-PRV-048 READY_TO_START or better | 03, 34, 45, packet | 2026-04-02 | registers ConsoleSink+RawLogSink+NormalizedEventSink for Cline runs; adds ClineEventExtractor sink in adapters/cline.py; shared harness must not be modified |
...
| PKT-PRV-061 | Config-driven canonical tag names | READY_TO_START | Infrastructure | workspace | needs PKT-PRV-059 VERIFIED | packet | 2026-04-02 | rename canonical tags to ag-* prefix; load ALLOWED_TAGS from prompt-syntax.yaml; add ag-short aliases; keep old names as backward-compat aliases |
| PKT-PRV-062 | Tag surface regeneration tool | READY_TO_START | Infrastructure | workspace | needs PKT-PRV-061 READY_TO_START or better | packet | 2026-04-02 | tools/regenerate_tag_surfaces.py reads prompt-syntax.yaml and regenerates all provider skill/instruction surfaces; --dry-run and --check modes |
| PKT-PRV-063 | Template installation documentation | READY_TO_START | Infrastructure | workspace | needs PKT-PRV-062 READY_TO_START or better | packet | 2026-04-02 | spec 50 documents required-managed vs create-if-missing vs generated-managed vs runtime-only; managed files get header comment; check_baseline_assets.py gains --check-managed-headers |
