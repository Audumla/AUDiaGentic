...
Document and isolate the provider-specific execution model for Qwen Code, without changing the shared canonical tag grammar.
...
3. Update only the provider-specific execution notes, settings profile, and smoke-test guidance for Qwen Code.
...
- the provider-specific execution doc is isolated to Qwen Code
...
experimental hook path; feature-flagged native intercept with shared policy
