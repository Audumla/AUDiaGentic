...
Implement the local-openai adapter against the provider contract.
...
```python
def run_local_openai(packet_ctx, provider_cfg):
    req = build_model_request(packet_ctx, provider_cfg)
    resp = call_openai_compatible_api(req, provider_cfg)
...
- local-openai adapter implemented
