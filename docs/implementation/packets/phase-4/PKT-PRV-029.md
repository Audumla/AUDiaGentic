# PKT-PRV-029 — local OpenAI-compatible tag execution implementation
...
Document and isolate the provider-specific execution model for local OpenAI-compatible backend, without changing the shared canonical tag grammar.
...
3. Update only the provider-specific execution notes, settings profile, and smoke-test guidance for local OpenAI-compatible backend.
...
- the provider-specific execution doc is isolated to local OpenAI-compatible backend
...
backend-only provider family; execution stays in the host client layer
