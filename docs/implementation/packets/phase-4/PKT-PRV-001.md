# PKT-PRV-001 — Provider registry and descriptor validation
...
Implement the registry of provider descriptors and canonical ids.
...
```python
def load_provider_registry(config):
    descriptors = parse_provider_descriptors(config)
    for d in descriptors: validate(d, schema="provider-descriptor")
    return {d["provider-id"]: d for d in descriptors}
```
...
- registry loads canonical provider ids and validates descriptors
