# PKT-PRV-002 — Provider selection and health check service
...
Implement provider selection, capability matching, and baseline health checks.
...
```python
def select_provider(job_req, registry):
    candidates = [p for p in registry.values() if satisfies_capabilities(p, job_req)]
    healthy = [p for p in candidates if healthcheck(p).status == "healthy"]
    if healthy: return rank_candidates(healthy, job_req)[0]
...
- selection chooses healthy provider or returns deterministic error
...
- provider selection service implemented
...
## Health-check and selection contract

This packet must implement:

```python
def health_check(provider_config) -> HealthCheckResult
def select_provider(job_record, provider_registry) -> str
```

### MVP selection rules

1. If the job explicitly names a provider-id, validate that provider first.
2. Reject the provider if required capabilities are missing.
3. Run baseline health check.
4. If unhealthy, fail with a provider-selection error in MVP rather than silently fail over.
5. DRAFT future enhancement: explicit failover policies.

### Test requirements

Add tests for:
- healthy selected provider
- missing capability
- unhealthy provider
- unknown provider-id
