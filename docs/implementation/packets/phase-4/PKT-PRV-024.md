...
Document and isolate the provider-specific execution model for Claude Code, without changing the shared canonical tag grammar.
...
3. Update only the provider-specific execution notes, settings profile, and smoke-test guidance for Claude Code.
...
- the provider-specific execution doc is isolated to Claude Code
...
Claude Code CLI / VS Code hook path; native intercept with tool gating
