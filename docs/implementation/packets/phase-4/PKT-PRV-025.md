...
Document and isolate the provider-specific execution model for Gemini, without changing the shared canonical tag grammar.
...
3. Update only the provider-specific execution notes, settings profile, and smoke-test guidance for Gemini.
...
- the provider-specific execution doc is isolated to Gemini
...
Gemini CLI / Code Assist hook path; native intercept with shared settings synchronization
