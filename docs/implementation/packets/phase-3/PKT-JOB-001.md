# PKT-JOB-001 — Job record store and state machine
...
Implement job persistence and legal transitions with no provider logic yet.
...
```python
def transition(job, new_state):
    allowed = LEGAL_TRANSITIONS[job.state]
    if new_state not in allowed:
        raise IllegalTransition(job.state, new_state)
    job.state = new_state
    save_job(job)
```
...
- job states enforce legal transitions only
...
- job store and state machine implemented
