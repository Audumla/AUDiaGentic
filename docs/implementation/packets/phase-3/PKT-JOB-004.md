# PKT-JOB-004 — Stage execution contract and stage output persistence
...
Implement the stage input/output wrapper used by all workflow stages.
...
```python
def execute_stage(job, stage, packet_ctx, prev_output=None):
    result = stage.handler(job, packet_ctx, prev_output)
    persist_stage_result(job.id, stage.id, result)
    return result
```
...
- stage result fixtures

## Tests to add

- stage results persisted and reusable by later stages
...
- stage execution contract implemented
