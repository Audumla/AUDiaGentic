# PKT-JOB-005 — Approvals and timeouts inside jobs
...
Integrate job state with the approval core and timeout behavior.
...
```python
def await_job_approval(job, approval_request):
    approval = create_approval(approval_request)
    transition(job, "awaiting-approval")
    while approval.pending and not approval.expired:
        sleep(POLL_INTERVAL)
        approval = refresh_approval(approval.id)
    return apply_approval_outcome(job, approval)
```
...
- jobs can enter awaiting-approval and recover from expiration
...
- job approval integration implemented
