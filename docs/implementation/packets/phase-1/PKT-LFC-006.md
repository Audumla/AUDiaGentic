# PKT-LFC-006 — Uninstall current AUDiaGentic
...
Implement uninstall of AUDiaGentic with the agreed preservation defaults and override flags.
...
- `tests/e2e/lifecycle/test_uninstall.py`
...
```python
def uninstall_current(ctx, remove_configs=False, remove_workflows=False):
    remove_runtime(ctx)
    if remove_configs: remove_project_configs(ctx)
    if remove_workflows: remove_managed_workflows(ctx)
    return success_result(uninstalled=True)
```
...
- configured project fixtures

## Tests to add

- default uninstall preserves tracked docs and project yaml per policy
...
- uninstall semantics verified in sandbox
