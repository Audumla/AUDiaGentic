# PKT-LFC-001 — Installed-state detector
...
Implement the detector that classifies project/install state before any destructive operation.
...
```python
def detect_installed_state(project_root):
    markers = collect_markers(project_root)
    if markers.none(): return "none"
    if markers.only_legacy(): return "legacy-only"
    if markers.only_audiagentic(): return "audiagentic"
...
- legacy/new/mixed fixtures

## Tests to add

- detector classifies none/legacy/audiagentic/mixed-invalid
...
- installed-state detection frozen
