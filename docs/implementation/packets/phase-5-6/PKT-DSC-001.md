# PKT-DSC-001 — Discord event subscriber and filter
...
Implement the event consumer/filter used by the Discord overlay.
...
```python
def should_publish_to_discord(event):
    return event["event-type"] in {"release-summary-updated","approval-requested","lifecycle-warning"}
```
...
- subscriber filters only allowed event kinds
...
- Discord event subscriber implemented
