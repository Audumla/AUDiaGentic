# PKT-DSC-004 — Discord lifecycle and migration warning publishing
...
Publish lifecycle warnings and migration summaries to Discord as optional operator notices.
...
```python
def publish_notice(event, discord_client):
    if event["severity"] in {"warning", "error"}:
        discord_client.post(render_notice(event))
```
...
- migration/lifecycle warnings can be surfaced when enabled
...
- Discord notice publishing implemented
