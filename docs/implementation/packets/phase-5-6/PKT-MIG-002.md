# PKT-MIG-002 — Provider migration rules and reports
...
Implement provider-config migration and reporting for cutover.
...
- `tests/integration/lifecycle/test_provider_migration.py`
...
```python
def migrate_provider_config(old_cfg):
    mapped = {}
    report = []
    for key, value in old_cfg.items():
        result = map_provider_field(key, value)
        mapped.update(result.mapped_fields)
...
- provider migration fixtures

## Tests to add

- provider migration report explains preserved/skipped/mapped fields
...
- provider migration rules implemented
