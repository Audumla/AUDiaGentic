# PKT-MIG-003 — Cutover recovery runbooks and troubleshooting
...
Write and validate operator runbooks for stuck cutover/update/uninstall states.

## Why this packet exists now

This packet is placed in Phase 5 6 because later packets depend on its outputs and must not redefine them.
...
- `docs/implementation/20_Cutover_Recovery_Runbooks.md`
- `tests/e2e/lifecycle/test_cutover_recovery.py`
...
```python
def build_recovery_runbook(checkpoint_state):
    if checkpoint_state.phase in PRE_DESTRUCTIVE:
        return instructions("safe_to_retry")
    if checkpoint_state.phase in POST_DESTRUCTIVE:
        return instructions("manual_recovery_required")
```
...
- cutover failure fixtures

## Tests to add

- runbooks cover each checkpoint failure state
...
- cutover recovery guidance complete
