# PKT-DSC-003 — Discord approval publishing and response handling
...
Publish approvals to Discord and convert responses back into approval core actions.
...
```python
def handle_discord_approval(response):
    approval_id = parse_approval_id(response)
...
- approval publish fixtures

## Tests to add

- approval responses feed approval core only
...
- Discord approval flow implemented
