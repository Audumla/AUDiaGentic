# PKT-FND-005 — Lifecycle CLI stub and checkpoint emitter
...
Implement the non-destructive CLI stub that all lifecycle packets will flesh out later.
...
```python
def lifecycle_stub(mode, project_root):
    ctx = build_stub_context(project_root)
    if mode == "plan":
        write_checkpoint(ctx, "detected")
        return emit_plan(ctx)
    if mode == "apply":
        write_checkpoint(ctx, "planned")
        write_checkpoint(ctx, "pre-destructive")
        return emit_apply_result(ctx)
    if mode == "validate":
        return emit_validation_result(ctx)
```
...
- lifecycle plan/apply/validate fixtures

## Tests to add

- stub writes expected checkpoint files
- no destructive filesystem changes outside temp sandbox
...
- deterministic CLI surface exists for later phases
...
Integration tests for this packet must use the destructive-test sandbox contract from `21_Destructive_Test_Sandbox.md`. The CLI stub must never mutate a non-sandbox working tree during tests.
