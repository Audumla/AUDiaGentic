# PKT-FND-004 — Example project scaffold
...
Create the reference project tree used by lifecycle, release, and migration tests.
...
```python
def seed_example_project(target):
    create_dirs(target, ["docs/architecture", "docs/implementation", "docs/releases", "docs/decisions", ".audiagentic/runtime"])
    write_yaml_examples(target)
    write_release_files(target)
```
...
- scaffold script creates deterministic layout
- example configs validate against schemas
...
- seedable example project exists
