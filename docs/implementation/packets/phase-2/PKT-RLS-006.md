# PKT-RLS-006 — Release Please baseline workflow and config management
...
Manage the baseline workflow/config files owned by AUDiaGentic.
...
```python
def ensure_release_please_baseline(ctx):
    state = detect_managed_workflow_state(ctx)
    if state == "absent": install_baseline(ctx)
    elif state == "legacy-detected": rename_legacy_then_install(ctx)
    elif state == "managed-unmodified": refresh_managed_files(ctx)
    else: preserve_and_warn(ctx)
```
...
- managed workflow/config detection and rename policy verified
...
- baseline Release Please management implemented
