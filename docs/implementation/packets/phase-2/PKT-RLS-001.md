# PKT-RLS-001 — Record fragment per change event
...
Implement fragment recording so every meaningful change becomes a runtime fragment before any tracked docs are touched.
...
```python
def record_change_event(event):
    validate(event, schema="change-event")
    path = build_fragment_path(event)
    atomic_write_json(path, event)
    return {"fragment-path": str(path), "event-id": event["event-id"]}
```
...
- record script writes one immutable fragment file per event
...
- fragment write path and naming frozen
