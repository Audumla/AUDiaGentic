...
# Notes
Completion update on 2026-04-19: interoperability event-layer foundation is implemented. Event bus, store, replay, async queue, planning integration, hook bridge, knowledge integration, audit tooling, and robustness follow-ups are landed. Deferred work such as broader CLI diagnostics and benchmarking was intentionally cancelled rather than left incomplete.
# Constraints
- Keep core operation dependency-light and in-process by default.
- Preserve explicit bootstrap wiring and avoid circular dependencies.
- Treat events as post-commit facts, not commands that own state changes.
- Preserve compatibility with existing components and Python 3.10+.
