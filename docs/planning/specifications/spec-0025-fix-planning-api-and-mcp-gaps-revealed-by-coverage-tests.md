...
Implement and verify the planning API and MCP fixes identified by the coverage-driven review in `request-0008`, keeping the coverage suites as the primary evidence source for completion.
...
1. Tasks `task-0202` through `task-0208` are complete and reflected as such in planning records only when corroborated by passing coverage tests.
