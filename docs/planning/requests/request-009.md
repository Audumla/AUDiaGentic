...
Exploratory test requests created during implementation review were consolidated here so the stack-profile feature has a single canonical request record. `request-008` remains a separate request because it tracks audience-level profile depth/detail rather than stack-topology behavior.
...
- Re-pointed linked implementation artifacts to `request-009` for traceability
- Preserved `request-008` as a separate future-facing request about audience-level profiles
