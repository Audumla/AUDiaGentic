...
This request is the current implementation-focused archive intake. It takes the earlier archive concept from `request-002` and narrows it into a concrete delivery slice for archive state support, state-driven archive/restore behavior, default archive filtering, validation behavior, and archive metadata visibility.
...
- `spec-001`, `plan-004`, and `wp-005` are the current implementation chain for this request.
- `request-002` remains the historical umbrella and provenance record for the earlier archive-workflow framing, but `request-004` is the active implementation intake.
...
- This request is a better build entrypoint than `request-002` because it is narrower and already aligned to the current task sequence (`task-0178` to `task-0182`).
