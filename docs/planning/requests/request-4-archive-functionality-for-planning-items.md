...
current_understanding: This request is the current implementation-focused archive
  ...
  request-23 as the active implementation intake while preserving that request as
...
- `request-2` remains the historical umbrella and provenance record for the earlier archive-workflow framing, but `request-4` is the active implementation intake.
...
- This request is a better build entrypoint than `request-2` because it is narrower and already aligned to the current task sequence (`task-0178` to `task-0182`).
