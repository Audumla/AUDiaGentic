...
summary: Verify relation-schema assumptions, implement soft delete and hard-delete
  counter safety, add bidirectional request traceability, prevent duplicates,
  improve validation feedback, and add integration coverage
plan_ref: plan-0010
task_refs:
- ref: task-0195
  seq: 1000
...
Deliver the focused planning-core hardening slice defined by `request-0007` and `spec-0024`: relation-schema verification, delete safety, correct counter behavior, traceability, duplicate prevention, clearer validation feedback, and integration coverage.

# Scope of This Package

- task-0195: Verify real relation schema contract and remove stale meta.task_refs assumption
- task-0196: Automatic counter sync after hard deletions
- task-0197: Soft delete functionality (tm_delete)
- task-0198: Auto-reference updates for plans and tasks
- task-0199: Duplicate detection for requests and specs
- task-0200: Improved schema validation error messages
- task-0201: Integration tests for all critical fixes

# Inputs

- spec-0024 (implementation specification)
- plan-0010 (execution plan)
- Existing codebase: `api.py`, `tm_helper.py`, `id_gen.py`, `val_mgr.py`

# Instructions

1. **task-0195**: Verify schema-compliant relation placement and remove stale planning assumptions
2. **task-0197**: Implement tm_delete() with soft/hard modes
3. **task-0196**: Integrate sync_counter() into hard delete operations
4. **task-0198**: Extend auto-reference updates to plans/tasks
5. **task-0199**: Add duplicate detection to prevent duplicate creation
6. **task-0200**: Improve validation error messages for better UX
7. **task-0201**: Add integration tests to verify all fixes work together
