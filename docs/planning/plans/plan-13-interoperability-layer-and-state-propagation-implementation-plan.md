...
- task-257: Package scaffolding and bootstrap wiring
- task-248: EventBus core (SYNC/ASYNC, cycle detection)
- task-249: FileEventStore (atomic writes, best-effort persistence)
- task-250: Replay service (is_replay flag, filtering)
- task-264: AsyncQueue (background worker, graceful shutdown)

## Phase 2: Planning Integration (Week 2)
- task-252: State propagation config system
- task-251: State propagation engine (rules, conflict resolution)
- task-253: Planning component integration (emit events)
- task-258: Event schema registry and documentation

## Phase 3: Cross-Component (Week 3)
- task-254: Knowledge component integration (subscribe to events)
- task-261: Knowledge event handler (sync actions on state changes)
- task-255: Hook migration (bridge legacy hooks to events)
- task-262: Hook inventory and migration plan
- task-263: Remove legacy hooks (cleanup)

## Phase 4: Tooling & Validation (Week 4)
- task-259: CLI commands (list, replay, debug, cleanup, audit)
- task-265: Planning audit and repair CLI
- task-260: Performance validation and benchmarking
- task-256: Integration tests (end-to-end scenarios)
...
- spec-19 must be `ready` before implementation starts ✅
- spec-20 must be `ready` before implementation starts ✅
...
- task-248: EventBus core
- task-249: FileEventStore
- task-250: Replay service
- task-264: AsyncQueue
- task-257: Package scaffolding

## WP-0024: State Propagation
- task-251: Propagation engine
- task-252: Config system
- task-253: Planning integration
- task-258: Event schema registry

## WP-0025: Cross-Component Integration
- task-254: Knowledge integration
- task-261: Knowledge event handler
- task-255: Hook migration
- task-262: Hook inventory
- task-263: Hook removal

## WP-0026: Tooling & Validation
- task-259: CLI commands
- task-265: Audit CLI
- task-260: Performance validation
- task-256: Integration tests
