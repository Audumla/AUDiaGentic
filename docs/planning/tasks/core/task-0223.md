...
Refactor three internal single-item read paths to use `lookup(id)` from task-0222 instead of calling `scan_items()`:
...
Depends on task-0221 (lookup.json) and task-0222 (lookup/head helpers). Bulk operations — `validate()`, `index()`, `status()` — still use `scan_items()` and are not in scope here.
