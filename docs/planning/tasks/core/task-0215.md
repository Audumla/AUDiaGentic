...
state: done
summary: Merge stack_profiles and request_profiles config into unified profiles structure, simplifying the CLI API from two parameters to one
spec_ref: spec-0038
request_refs:
- request-0010
---
...
**Profiles Provided:**
- `feature`: request defaults + spec→task cascade, allows overlay
- `issue`: request defaults + spec→task cascade, allows overlay
- `direct`: spec→task only, no overlay
- `full`: spec→task + overlay available

# Notes

This was a refinement discovered during initial implementation. The unification makes the API simpler and more discoverable: one parameter controls both request context and execution topology.
