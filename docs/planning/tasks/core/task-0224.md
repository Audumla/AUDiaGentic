...
Depends on task-0223 (extract already uses lookup path). The `with_related` and `with_resources` parameters already default to `False` and are not changed by this task.
