...
# Description
Implemented. Planning MCP now exposes read-only config discovery through `tm_docs op=config`, including compact and full modes, kind filtering, and live creation guidance for agents before `tm_create`.
# Notes
Polish update on 2026-04-19: request-30 considered complete. Follow-on optimization should be driven by real agent usage rather than more speculative expansion.
