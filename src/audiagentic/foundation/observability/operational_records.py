...
_OPR_DENYLIST_FIELDS = frozenset(
    (
        "prompt-body",
        "output",
        "prompt_body",
...
def append_operational_record(path: Path, record: dict[str, Any]) -> None:
    ...
    Requires:
        ...
        - No top-level denylisted field names (prompt-body, output, etc.).

    Raises:
        AudiaGenticError: VAL-OPR-001 if correlation_id is missing.
        AudiaGenticError: CON-OPR-002 if a denylisted key is present.
    """
    ...
    for denied in _OPR_DENYLIST_FIELDS:
        if denied in record:
