...
Completely agnostic about workflows and document relationships — all rules,
actions, and relationships are defined in the config file using semantic state sets.

This is a passive utility: it does NOT subscribe to events. The owner component
registers event handlers and calls propagate().

Architecture:
- Rules and actions are defined in config, implemented as functions in propagation_rules.py
- The engine only orchestrates — no business logic
...
class StatePropagationEngine:
    ...
    def __init__(
        ...
        Args:
            ctx: PlanningAPI instance for item lookup
        ...
        """
        self._ctx: PlanningAPIInterface = ctx
    ...
    def config(self) -> Any:
        ...
        return getattr(self._ctx, "config", None)
    ...
    def _get_item_with_kind(self, item_id: str) -> tuple[Any, str | None]:
        ...
        """
        item_view = self._ctx.lookup(item_id)
    ...
    def apply_propagation(
        ...
        target_view = self._ctx.lookup(target_id)
    ...
    def _propagation_log_path(self) -> Path | None:
        root = getattr(self._ctx, "root", None)
        if root is None:
            return None
    ...
    def _log_propagation_attempt(
        ...
        *,
        ...
        source_view = self._ctx.lookup(source_id)
    ...
    def _linked_child_ids(
        ...
        for item_view in self._ctx._scan():
        ...
        for parent_id, found_parent_kind in parents:
            ...
            if parent_is_complete:
                ...
                for ref in child_refs:
                    ...
                    child_view = self._ctx.lookup(ref_id)
    ...
    def _apply_fix(self, fix: dict[str, Any]) -> None:
        ...
        self._ctx.state(id_=target_id, new_state=target_state, metadata=metadata)
