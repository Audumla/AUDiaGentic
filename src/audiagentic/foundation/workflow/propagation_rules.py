...
def rule_parent_in_set(
    ...
    """
    parent_view = engine._ctx.lookup(parent_id)
...
def rule_all_children_in_set(
    ...
    child_view = engine._ctx.lookup(child_id)
    ...
    for cid in child_ids:
        sibling_view = engine._ctx.lookup(cid)
...
def action_complete_parent(
    ...
    """
    source_view = engine._ctx.lookup(item_id)
    ...
    if not engine.config.state_in_set(
        source_kind,
        source_view.data.get("state"),
        required_state_set,
        source_view.data.get("workflow"),
    ):
        return []
    ...
    for parent_id in parent_refs:
        ...
        for sibling_id in sibling_ids:
            sibling_view = engine._ctx.lookup(sibling_id)
            if not sibling_view or not sibling_view.data:
