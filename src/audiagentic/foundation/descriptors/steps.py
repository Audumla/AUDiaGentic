"""Workflow step builder for descriptor YAML.
...
def build_toolchain_step(toolchain: str, action: str, package: str, extra: list[str] | None = None) -> ShellStep | SequenceStep:
    """Build a step from a toolchain name, action, and package.

    Delegates to foundation/toolchains/loader.build_step. Returns
    ShellStep or SequenceStep (never CallableStep or SelectStep).

    Args:
        toolchain: Toolchain identifier (e.g. 'npm', 'uv', 'brew').
        action: Action name ('install', 'uninstall', 'remove', 'probe').
        package: Package name for the toolchain.
        extra: Extra arguments appended to the command.

    Returns:
        ShellStep or SequenceStep from the toolchain loader.
    """
    from audiagentic.foundation.toolchains.loader import build_step as _tc_build_step

    return _tc_build_step(toolchain, action, package, *(extra or []))
