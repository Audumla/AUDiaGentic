"""Generic descriptor loader with YAML file support.

Provides the mechanism used by all descriptor types:
...
Field handling kinds:
    ...
    - step: workflow step spec dict, built via steps.py
    - nested: sub-dict mapped by a builder function

Error codes:
    VAL-DESC-001 — ref resolution failure (from resolver)
    VAL-DESC-002 — step build failure (from steps)
...
from .steps import build_step_from_spec
