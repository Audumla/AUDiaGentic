"""Public in-process client for the agent LLM gateway.
...
class InProcessGatewayClient:
    ...
    def submit_llm_request(self, project_root: Path, **kwargs: Any) -> dict[str, Any]:
        return self._application.submit_llm_request(project_root, **kwargs)

    def get_llm_request(self, project_root: Path, request_id: str) -> dict[str, Any]:
        return self._application.get_llm_request(project_root, request_id)

    def wait_llm_request(self, project_root: Path, request_id: str, timeout_seconds: float | None = None) -> dict[str, Any]:
        return self._application.wait_llm_request(project_root, request_id, timeout_seconds)

    def cancel_llm_request(self, project_root: Path, request_id: str) -> dict[str, Any]:
        return self._application.cancel_llm_request(project_root, request_id)

    def request_runtime_status(self, project_root: Path, request_id: str) -> dict[str, Any]:
        return self._application.request_runtime_status(project_root, request_id)

    def run_llm_request(self, project_root: Path, **kwargs: Any) -> dict[str, Any]:
        return self._application.run_llm_request(project_root, **kwargs)

    def list_llm_requests(self, project_root: Path, **kwargs: Any) -> list[dict[str, Any]]:
        return self._application.list_llm_requests(project_root, **kwargs)

    def gateway_overview(self, project_root: Path) -> dict[str, Any]:
        return self._application.gateway_overview(project_root)

    def list_llm_sessions(self, project_root: Path, **kwargs: Any) -> list[dict[str, Any]]:
        return self._application.list_llm_sessions(project_root, **kwargs)

    def close_llm_session(self, project_root: Path, session_id: str) -> dict[str, Any]:
        return self._application.close_llm_session(project_root, session_id)

    def resume_llm_session(
        self, project_root: Path, source_session_id: str, **kwargs: Any
    ) -> dict[str, Any]:
        return self._application.resume_llm_session(project_root, source_session_id, **kwargs)
