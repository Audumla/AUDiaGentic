...
function activityLabel(r) { const a=r.activity||{}; const phase=a.phase||a['last-source']||r['activity-source']||''; const seq=r['activity-sequence']; return `${esc(phase)}${seq!==undefined?` <span class="muted">#${esc(seq)}</span>`:''}`; }
