...
def _session_row(record: dict[str, Any], live: dict[str, Any] | None) -> dict[str, Any]:
    ...
    chat_title = provider_metadata.get("chat-title")
...
.project { padding:16px; margin:14px 0; container-type:inline-size; --request-cols:minmax(0,1.55fr) minmax(82px,.72fr) minmax(0,1.15fr) minmax(116px,.95fr) minmax(0,1.35fr) }.project-head { margin-bottom:12px }.project-name { display:flex; align-items:center; gap:8px; font-weight:750; font-size:15px }.project-name .icn { color:var(--purple) }.pills { display:flex; gap:6px; flex-wrap:wrap }.pill { border-radius:999px; padding:3px 9px; font-size:11px; font-weight:650; border:1px solid var(--line); color:var(--muted) }.pill.state-completed { color:var(--green); border-color:#254a37 }.pill.state-failed,.pill.state-rejected,.pill.state-interrupted,.pill.state-timed-out,.pill.state-expired,.pill.state-abandoned { color:var(--red); border-color:#5a2a30 }.pill.state-queued,.pill.state-running,.pill.state-dispatching,.pill.state-active { color:var(--amber); border-color:#5a4a24 }
#projects.layout-columns { display:grid; grid-template-columns:repeat(auto-fit,minmax(min(100%,460px),1fr)); gap:14px; align-items:start } #projects.layout-columns .project { margin:0; min-width:0 } #projects.layout-rows { display:block } .work-section { margin-top:12px } .work-section > h3 { color:var(--text); font-size:12px; letter-spacing:.1em; text-transform:uppercase; margin:0 0 8px; padding:7px 9px; border-bottom:2px solid #3b5278; font-weight:750 } .request-grid { display:grid; gap:6px } .request-header,.request-row { display:grid; grid-template-columns:var(--request-cols); gap:8px; align-items:center } .request-header { color:var(--muted); font-size:10px; font-weight:650; letter-spacing:.06em; text-transform:uppercase; padding:0 9px 2px } .request-header > div { display:flex; align-items:center; min-width:0; min-height:24px } .request-header > div,.request-row > div { min-width:0; overflow-wrap:anywhere } .request-row { padding:8px 9px; border:1px solid #263757; border-radius:8px; background:#101a30 } .request-row .request-identity { display:flex; gap:5px; align-items:center; flex-wrap:nowrap; min-width:0 } .request-row .request-id { min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap } .request-row .request-title { color:var(--text); font-size:12px; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap } .request-row .request-meta { color:var(--muted); font-size:12px; min-width:0 } .request-row .request-error { color:var(--red); font-size:12px; min-width:0 } .session-actions { display:flex; gap:5px; align-items:center; justify-content:flex-end; min-width:0 } .icon-button { min-width:30px !important; width:30px; height:28px; flex:0 0 30px; padding:4px !important; font-size:16px !important } .icon-button svg { width:15px; height:15px; fill:none; stroke:currentColor; stroke-width:1.8; stroke-linecap:round; stroke-linejoin:round } .icon-button.feedback { border-color:var(--teal); color:var(--teal) }
.queue { font:12px ui-monospace,monospace; color:var(--muted); margin-top:2px }.sessions { display:grid; gap:10px }.session { overflow:hidden; position:relative; border-left:3px solid var(--line) }.session.state-completed { border-left-color:var(--green) }.session.state-failed,.session.state-rejected,.session.state-interrupted,.session.state-timed-out,.session.state-expired,.session.state-abandoned { border-left-color:var(--red) }.session.state-queued,.session.state-running,.session.state-dispatching,.session.state-active { border-left-color:var(--amber) }.session summary { cursor:pointer; list-style:none; padding:12px 14px }.session summary::-webkit-details-marker { display:none }.session-head { display:grid; grid-template-columns:minmax(0,1fr) auto; align-items:center }.session-body { border-top:1px solid var(--line); padding:0 14px 12px }
.request-row .request-id { flex:0 1 auto } .request-row .request-title { flex:1 1 auto } .request-row .request-execution { color:var(--muted); font-size:11px; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap } .icon-button { min-width:26px !important; width:26px; height:26px; flex:0 0 26px }
...
@container (max-width:600px) { .session-head { grid-template-columns:1fr } .session-actions { justify-content:flex-start } .request-header,.request-row { grid-template-columns:minmax(0,1.5fr) minmax(80px,.75fr) minmax(0,1fr) 112px } .request-error { grid-column:1/-1 } }
</style>
...
function activityLabel(r) { const phase=r['activity-type']||''; const fallback=phase||(r.state==='queued'?'waiting':(ACTIVE_REQUEST_STATES.has(r.state)?r.state:'')); const seq=r['activity-sequence']; if(!fallback)return ''; return `<span class="activity-badge"><span class="activity-kind">${esc(fallback)}</span>${seq!==undefined?`<span class="activity-seq">#${esc(seq)}</span>`:''}</span>`; }
const focusEndpoint=__FOCUS_PATH__; const purgeEndpoint=__PURGE_PATH__; const focusToken=__FOCUS_TOKEN__;
const focusIcon='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 4h14a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2Z"/><path d="M3 8h18"/><path d="M8 14h8"/><path d="m13 11 3 3-3 3"/></svg>';
...
function sessionCard(session, rows, empty=false, allRows=rows) { const purgeable=!allRows.some(r=>ACTIVE_REQUEST_STATES.has(r.state))&&!session['turn-active']&&!(session['pending-turns']>0); const purge=purgeable?`<button type="button" class="action-button icon-button purge-session" data-session-id="${esc(session['session-id'])}" aria-label="Purge session" title="Permanently remove this session and all gateway data">${purgeIcon}</button>`:''; const execution=executionSummary(esc(session['execution-profile-id']||''),esc(session['provider-id']||''),esc(session['model-id']||'')); const title=session['provider-chat-title']?` <span class="session-profile" title="${esc(session['provider-chat-title'])}">${esc(session['provider-chat-title'])}</span>`:''; return `<details class="session ${stateClass(session.state)}" open><summary><div class="session-head"><div class="session-identity"><h3><code>${esc(session['session-id'])}</code>${execution?` <span class="session-profile">${execution}</span>`:''}${title} ${session.live?'<span class="live">live</span>':''}</h3></div><div class="session-actions">${badge(session.state)} ${session['turn-active']?'<span class="live">turn active</span>':''}<span class="muted">${session['turn-count']||0} turns · ${session['pending-turns']||0} pending</span>${purge}</div></div></summary><div class="session-body"><div class="request-grid">${empty?'<div class="muted">No matching requests</div>':requestRows(rows,false)}</div></div></details>`; }
...
function projectView(project) {
  ...
  const section=(title,groups)=>{if(!groups.length)return ''; return `<section class="work-section"><h3>${title}</h3>${requestHeader()}<div class="sessions">${groups.map(([s,rows,allRows])=>s?sessionCard(s,rows,false,allRows):`<div class="orphan"><div class="request-grid">${requestRows(rows)}</div></div>`).join('')}</div></section>`};
