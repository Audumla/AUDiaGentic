...
.badge { display:inline-block; border:1px solid var(--line); border-radius:999px; padding:2px 8px; font-size:12px; white-space:nowrap }.activity-badge { display:inline-flex; align-items:center; gap:5px; border:1px solid #2b5665; border-radius:999px; padding:2px 8px; color:var(--teal); background:#102938; font-size:12px; line-height:1.25; white-space:nowrap }.activity-seq { color:var(--muted); font-size:11px }.state-completed { color:var(--green) }.state-failed,.state-rejected,.state-interrupted { color:var(--red) }.state-queued,.state-running,.state-dispatching,.state-active { color:var(--amber) }.live { color:var(--amber); font-size:12px; font-weight:650 }.session-profile { color:var(--muted); font-size:11px; font-weight:500 } code { color:var(--teal); font:12px ui-monospace,SFMono-Regular,Consolas,monospace }
...
function activityLabel(r) { const phase=r['activity-type']||''; const fallback=phase||(r.state==='queued'?'waiting':(ACTIVE_REQUEST_STATES.has(r.state)?r.state:'')); const seq=r['activity-sequence']; if(!fallback)return ''; return `<span class="activity-badge"><span>${esc(fallback)}</span>${seq!==undefined?`<span class="activity-seq">#${esc(seq)}</span>`:''}</span>`; }
...
function executionSummary(profile, provider, model) { const names=[]; [profile,provider].forEach(name=>{if(name && !names.includes(name))names.push(name);}); const left=names.join(' · '); return left+(model?(left?' / ':'')+model:''); }
...
function sessionCard(session, rows, empty=false, allRows=rows) { const purgeable=!allRows.some(r=>ACTIVE_REQUEST_STATES.has(r.state))&&!session['turn-active']&&!(session['pending-turns']>0); const purge=purgeable?`<button type="button" class="action-button icon-button purge-session" data-session-id="${esc(session['session-id'])}" aria-label="Purge session" title="Permanently remove this session and all gateway data">${purgeIcon}</button>`:''; const profile=esc(session['execution-profile-id']||''); const providerModel=executionSummary('',esc(session['provider-id']||''),esc(session['model-id']||'')); return `<details class="session ${stateClass(session.state)}" open><summary><div class="session-head"><div><h3><code>${esc(session['session-id'])}</code>${profile?` <span class="session-profile">${profile}</span>`:''} ${session.live?'<span class="live">live</span>':''}</h3>${providerModel?`<span class="muted">${providerModel}</span>`:''}</div><div>${badge(session.state)} ${session['turn-active']?'<span class="live">turn active</span>':''}</div><div class="session-actions"><span class="muted">${session['turn-count']||0} turns · ${session['pending-turns']||0} pending</span>${purge}</div></div></summary><div class="session-body"><div class="request-grid">${empty?'<div class="muted">No matching requests</div>':requestRows(rows,false)}</div></div></details>`; }
