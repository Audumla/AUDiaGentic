...
input[type=number],button { background:#0b1529; border:1px solid var(--line); border-radius:7px; color:var(--text); padding:6px 8px } input[type=number] { width:100px } button { cursor:pointer } button:hover { border-color:var(--teal) } .chat-link { color:var(--teal); font-size:11px; white-space:nowrap; text-decoration:none } .chat-link:hover { text-decoration:underline }
...
const focusEndpoint=__FOCUS_PATH__; const focusToken=__FOCUS_TOKEN__;
function chatLink(r) { const url=r['provider-chat-url']; if(!url) return ''; const target='audiagentic-gpt-'+String(r['request-id']||'').replace(/[^A-Za-z0-9_-]/g,'_'); return ` <a class="chat-link" href="${esc(url)}" target="${esc(target)}" rel="noopener" title="Open the retained GPT chat URL">Open GPT chat ↗</a> <button type="button" class="focus-chat" data-request-id="${esc(r['request-id'])}" title="Focus the existing GPT tab">Focus tab</button>`; }
...
function bindFocusButtons() { document.querySelectorAll('.focus-chat').forEach(button=>button.addEventListener('click',()=>focusChat(button))); }
function requestRows(rows) { return rows.sort(byNewest).map(r=>{const d=r.diagnostics||{}; return `<tr><td><code>${esc(r['request-id'])}</code>${chatLink(r)}</td><td>${badge(r.state)}${r['provider-turn-pending']?' <span class="live">unresolved</span>':''}${watchdogFlag(r)}</td><td>${activityLabel(r)}</td><td>${esc(r['execution-profile-id']||'')} / ${esc(r['resolved-provider-id']||'')}</td><td>${stamp(recent(r))}</td><td class="error">${esc(d.classification||r.error?.code||'')} ${esc(d['recovery']?.disposition||r.error?.message||'')}</td></tr>`}).join('')||'<tr><td colspan="6" class="muted">No matching requests</td></tr>'; }
...
</script></main>""".replace("__SNAPSHOT_PATH__", source).replace("__FOCUS_PATH__", focus_source).replace("__FOCUS_TOKEN__", token_source)
