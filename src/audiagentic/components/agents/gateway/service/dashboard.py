...
function chatLink(r) { if(!r['focus-tab-available']) return ''; return ` <button type="button" class="action-button focus-chat" data-request-id="${esc(r['request-id'])}" title="Open the retained GPT tab, or focus it if already open">Open / focus tab</button>`; }
async function focusChat(button) { const id=button.dataset.requestId; button.disabled=true; try { const response=await fetch(focusEndpoint,{method:'POST',headers:{'Content-Type':'application/json','X-AudiaGentic-Dashboard-Token':focusToken},body:JSON.stringify({'request-id':id})}); const body=await response.json(); const result=body.result||{}; button.textContent=result.reason==='conversation-tab-opened'?'Opened':(result.outcome==='focused'?'Focused':(result.reason||result.outcome||'Unavailable')); } catch(error) { button.textContent='Unavailable'; } finally { setTimeout(()=>{button.disabled=false;},1200); } }
...
function requestRows(rows, includeExecution=true) { return rows.sort(byNewest).map(r=>{const d=r.diagnostics||{}; const executionCell=includeExecution?`<td>${esc(r['execution-profile-id']||'')} / ${esc(r['resolved-provider-id']||'')}</td>`:''; return `<tr><td><code>${esc(r['request-id'])}</code>${chatLink(r)}</td><td>${badge(r.state)}${r['provider-turn-pending']?' <span class="live">unresolved</span>':''}${sideEffectFlag(r)}${watchdogFlag(r)}</td><td>${activityLabel(r)}</td>${executionCell}<td>${stamp(recent(r))}</td><td class="error">${esc(d.classification||r.error?.code||'')} ${esc(d['recovery']?.disposition||r.error?.message||'')}</td></tr>`}).join('')||`<tr><td colspan="${includeExecution?6:5}" class="muted">No matching requests</td></tr>`; }
...
function sessionCard(session, rows) { return `<details class="session ${stateClass(session.state)}" open><summary><div class="session-head"><div><h3><code>${esc(session['session-id'])}</code> ${session.live?'<span class="live">live</span>':''}</h3><span class="muted">${esc(session['execution-profile-id']||'')}${session['provider-id']?' · '+esc(session['provider-id']):''}${session['model-id']?' / '+esc(session['model-id']):''}</span></div><div>${badge(session.state)} ${session['turn-active']?'<span class="live">turn active</span>':''}</div><div class="muted">${session['turn-count']||0} turns · ${session['pending-turns']||0} pending · ${stamp(recent(session))} <button type="button" class="action-button purge-session" data-session-id="${esc(session['session-id'])}" title="Permanently remove this session and all gateway data">Purge</button></div></div></summary><div class="session-body"><table class="request-table session-request-table"><thead><tr><th>Request</th><th>State</th><th>Activity</th><th>Updated</th><th>Error</th></tr></thead><tbody>${requestRows(rows,false)}</tbody></table></div></details>`; }
function projectView(project) { const grouped=new Map(); (project.requests||[]).forEach(r=>{const id=r['session-id']; if(id){const existing=grouped.get(id)||[]; existing.push(r); grouped.set(id,existing)}}); const sessions=(project.sessions||[]).filter(s=>{const rows=grouped.get(s['session-id'])||[]; const hasRequests=rows.length>0; return (showClosed.checked||!isClosed(s)||hasRequests)&&(showEmpty.checked||hasRequests)&&matchesState(s,rows)}).sort(byNewest); const oneShots=(project.requests||[]).filter(r=>!r['session-id']&&matchesRequest(r)).sort(byNewest); if(!sessions.length&&!oneShots.length)return ''; const tally={}; sessions.forEach(s=>{tally[s.state]=(tally[s.state]||0)+1}); const pills=Object.entries(tally).map(([state,n])=>`<span class="pill ${stateClass(state)}">${n} ${esc(state)}</span>`).join('')+(oneShots.length?`<span class="pill">${oneShots.length} one-shot</span>`:''); const oneShotSection=oneShots.length?`<section class="orphan"><h3>One-shot requests</h3><table class="request-table one-shot-request-table"><thead><tr><th>Request</th><th>State</th><th>Activity</th><th>Profile / provider</th><th>Updated</th><th>Error</th></tr></thead><tbody>${requestRows(oneShots)}</tbody></table></section>`:''; return `<section class="project"><div class="project-head"><div><div class="project-name"><span class="icn">▣</span>${esc(project.name)}</div><div class="queue">${esc(queueSummary(project.queues))}</div></div><div class="pills">${pills}</div></div><div class="sessions">${sessions.map(s=>sessionCard(s,grouped.get(s['session-id'])||[])).join('')}</div>${oneShotSection}</section>`; }
