...
@container (max-width:440px) { .request-header,.request-row { grid-template-columns:minmax(0,1.45fr) minmax(78px,.75fr) minmax(0,1fr) 108px } .request-header > :nth-child(5) { display:none } .request-error { grid-column:1/-1 } }
</style>
...
const dateTimeFmt=new Intl.DateTimeFormat(undefined,{dateStyle:'short',timeStyle:'short'}); const clockFmt=new Intl.DateTimeFormat(undefined,{timeStyle:'medium'}); const stamp=x=>{if(x===undefined||x===null||x===''||(typeof x==='number'&&!Number.isFinite(x)))return ''; const d=new Date(x); return Number.isFinite(d.getTime())?dateTimeFmt.format(d):''}; const isClosed=s=>['closed','expired'].includes(s?.state); const timestamp=x=>{const value=Date.parse(x||''); return Number.isFinite(value)?value:-Infinity}; const recent=x=>Math.max(...['updated-at','last-activity-at','closed-at','created-at'].map(key=>timestamp(x?.[key])));
...
function sideEffectFlag(r) { const side=r.diagnostics?.['side-effect-state']; if(r.state==='completed'||((!FAILED_REQUEST_STATES.has(r.state)&&r.state!=='cancelled')||side!=='may-have-started')) return ''; return `<span class="flag warn" title="The provider may have received this turn; verify the provider session before retrying.">turn uncertain</span>`; }
...
function bindSessionToggles() { document.querySelectorAll('details.session[data-session-id]').forEach(session=>session.addEventListener('toggle',()=>{ const id=session.dataset.sessionId; if(!id)return; if(session.open) collapsedSessionIds.delete(id); else collapsedSessionIds.add(id); persistCollapsedSessions(); })); }
...
function requestRows(rows, includeExecution=true) { const ordered=rows.slice().sort(byRequestNewest); if(!ordered.length)return `<div class="muted">No matching requests</div>`; return ordered.map(r=>{const d=r.diagnostics||{}; const execution=includeExecution?executionSummary(esc(r['execution-profile-id']||''),esc(r['resolved-provider-id']||''),esc(r['resolved-model-id']||'')):''; const error=esc(d.classification||r.error?.code||'')+' '+esc(d['recovery']?.disposition||r.error?.message||''); const title=includeExecution&&r['provider-chat-title']?`<span class="request-title" title="${esc(r['provider-chat-title'])}">${esc(r['provider-chat-title'])}</span>`:''; return `<div class="request-row"><div class="request-identity"><code class="request-id">${esc(r['request-id'])}</code>${title}${execution?`<span class="request-execution" title="${execution}">${execution}</span>`:''}${chatLink(r)}</div><div>${badge(r.state)}${sideEffectFlag(r)}</div><div class="request-meta">${activityLabel(r)||'<span class="muted">none</span>'}</div><div class="request-meta request-updated">${stamp(recent(r))}</div><div class="request-error">${error||'<span class="muted">none</span>'}</div></div>`}).join(''); }
...
function projectView(project) {
  ...
  Object.values(sections).forEach(groups=>groups.sort(byGroupNewest)); if(!sections.active.length&&!sections.completed.length&&!sections.failed.length&&!emptySessions.length)return '';
  ...
  const section=(title,groups)=>{if(!groups.length)return ''; const requestCount=groups.reduce((count,group)=>count+group[1].length,0); const sectionClass=title.toLowerCase(); const orderedGroups=groups.slice().sort(title==='Active'?byGroupNewest:byGroupUpdated); return `<section class="work-section work-section-${sectionClass}"><div class="work-section-head"><h3>${title}</h3><span class="section-count">${requestCount}</span></div>${requestHeader()}<div class="sessions">${orderedGroups.map(([s,rows,allRows])=>s?sessionCard(s,rows,false,allRows):`<div class="orphan"><div class="request-grid">${requestRows(rows)}</div></div>`).join('')}</div></section>`};
  ...
  return `<section class="project"><div class="project-head"><div><div class="project-name"><span class="icn"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3.5 6.5h6l2 2h9v9a2 2 0 0 1-2 2h-15z"/><path d="M3.5 6.5v-1a2 2 0 0 1 2-2h4l2 2h5"/></svg></span>${esc(project.name)}</div><div class="queue">${esc(queueSummary(project.queues))}</div></div><div class="pills">${pills}</div></div>${section('Active',sections.active)}${section('Completed',sections.completed)}${section('Failed',sections.failed)}${empty}</section>`;
}
...
function draw(snapshot) {
  ...
  bindFocusButtons(); bindSessionToggles();
