...
function executionSummary(profile, provider, model) { const names=[]; [profile,provider].forEach(name=>{if(name && !names.includes(name))names.push(name);}); const left=names.join(' · '); const distinctModel=model&&!names.includes(model)?model:''; return left+(distinctModel?(left?' / ':'')+distinctModel:''); }
function requestRows(rows, includeExecution=true) { const ordered=rows.slice().sort(byNewest); if(!ordered.length)return `<div class="muted">No matching requests</div>`; return requestHeader(includeExecution)+ordered.map(r=>{const d=r.diagnostics||{}; const execution=includeExecution?executionSummary(esc(r['execution-profile-id']||''),esc(r['resolved-provider-id']||''),''):''; const error=esc(d.classification||r.error?.code||'')+' '+esc(d['recovery']?.disposition||r.error?.message||''); return `<div class="request-row"><div class="request-identity"><code class="request-id">${esc(r['request-id'])}</code>${chatLink(r)}</div><div>${badge(r.state)}${sideEffectFlag(r)}</div><div class="request-meta">${activityLabel(r)||'<span class="muted">none</span>'}</div>${includeExecution?`<div class="request-meta">${execution}</div>`:'<div class="request-meta">'+stamp(recent(r))+'</div>'}<div class="request-error">${includeExecution?stamp(recent(r))+' · ':''}${error||'<span class="muted">none</span>'}</div></div>`}).join(''); }
...
function sessionCard(session, rows, empty=false, allRows=rows) { const purgeable=!allRows.some(r=>ACTIVE_REQUEST_STATES.has(r.state))&&!session['turn-active']&&!(session['pending-turns']>0); const purge=purgeable?`<button type="button" class="action-button icon-button purge-session" data-session-id="${esc(session['session-id'])}" aria-label="Purge session" title="Permanently remove this session and all gateway data">${purgeIcon}</button>`:''; const execution=executionSummary(esc(session['execution-profile-id']||''),esc(session['provider-id']||''),esc(session['model-id']||'')); return `<details class="session ${stateClass(session.state)}" open><summary><div class="session-head"><div class="session-identity"><h3><code>${esc(session['session-id'])}</code>${execution?` <span class="session-profile">${execution}</span>`:''} ${session.live?'<span class="live">live</span>':''}</h3></div><div class="session-actions">${badge(session.state)} ${session['turn-active']?'<span class="live">turn active</span>':''}<span class="muted">${session['turn-count']||0} turns · ${session['pending-turns']||0} pending</span>${purge}</div></div></summary><div class="session-body"><div class="request-grid">${empty?'<div class="muted">No matching requests</div>':requestRows(rows,false)}</div></div></details>`; }
