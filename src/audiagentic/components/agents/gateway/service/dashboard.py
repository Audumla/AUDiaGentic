...
#projects.layout-columns { display:grid; grid-template-columns:repeat(auto-fit,minmax(min(100%,460px),1fr)); gap:14px; align-items:start } #projects.layout-columns .project { margin:0; min-width:0 } #projects.layout-rows { display:block } .work-section { margin-top:12px } .work-section > h3 { color:var(--muted); font-size:11px; letter-spacing:.1em; text-transform:uppercase; margin:0 0 7px } .request-grid { display:grid; gap:6px } .request-row { display:grid; grid-template-columns:minmax(150px,1.3fr) minmax(105px,.8fr) minmax(125px,1fr) minmax(125px,1fr) minmax(140px,1.4fr); gap:8px; align-items:start; padding:8px 9px; border:1px solid #263757; border-radius:8px; background:#101a30 } .request-row > div { min-width:0; overflow-wrap:anywhere } .request-row .request-id { display:flex; gap:5px; align-items:flex-start; flex-wrap:wrap } .request-row .request-meta { color:var(--muted); font-size:12px } .request-row .request-error { color:var(--red); font-size:12px } .request-row .request-label { color:var(--muted); font-size:10px; text-transform:uppercase; letter-spacing:.06em; display:block } .session-actions { display:flex; gap:5px; align-items:center; justify-content:flex-end } .icon-button { min-width:30px !important; width:30px; height:28px; padding:4px !important; font-size:16px !important } .icon-button svg { width:15px; height:15px; fill:none; stroke:currentColor; stroke-width:1.8; stroke-linecap:round; stroke-linejoin:round } .icon-button.feedback { border-color:var(--teal); color:var(--teal) }
...
const focusIcon='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4H4v4M4 4l7 7M16 20h4v-4M20 20l-7-7M20 8V4h-4M20 4l-7 7M4 16v4h4M4 20l7-7"/></svg>';
const purgeIcon='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M10 11v6M14 11v6M6 7l1 13h10l1-13M9 7V4h6v3"/></svg>';
function chatLink(r) { if(!r['focus-tab-available']) return ''; return ` <button type="button" class="action-button icon-button focus-chat" data-request-id="${esc(r['request-id'])}" aria-label="Open or focus GPT tab" title="Open the retained GPT tab, or focus it if already open">${focusIcon}</button>`; }
...
function requestRows(rows, includeExecution=true) { return rows.slice().sort(byNewest).map(r=>{const d=r.diagnostics||{}; const execution=includeExecution?`${esc(r['execution-profile-id']||'')} / ${esc(r['resolved-provider-id']||'')}`:''; const error=esc(d.classification||r.error?.code||'')+' '+esc(d['recovery']?.disposition||r.error?.message||''); return `<div class="request-row"><div class="request-id"><span class="request-label">Request</span><code>${esc(r['request-id'])}</code>${chatLink(r)}</div><div><span class="request-label">State</span>${badge(r.state)}${r['provider-turn-pending']?' <span class="live">unresolved</span>':''}${sideEffectFlag(r)}${watchdogFlag(r)}</div><div class="request-meta"><span class="request-label">Activity</span>${activityLabel(r)||'<span class="muted">none</span>'}</div>${includeExecution?`<div class="request-meta"><span class="request-label">Profile / provider</span>${execution}</div>`:'<div class="request-meta"><span class="request-label">Updated</span>'+stamp(recent(r))+'</div>'}<div class="request-error"><span class="request-label">${includeExecution?'Updated / error':'Error'}</span>${includeExecution?stamp(recent(r))+' · ':''}${error||'<span class="muted">none</span>'}</div></div>`}).join('')||`<div class="muted">No matching requests</div>`; }
...
function projectView(project) { const grouped=new Map(); const requests=(project.requests||[]); requests.forEach(r=>{const id=r['session-id']; if(id){const existing=grouped.get(id)||[]; existing.push(r); grouped.set(id,existing)}}); const knownSessionIds=new Set((project.sessions||[]).map(s=>s['session-id'])); const sessions=(project.sessions||[]).filter(s=>{const allRows=grouped.get(s['session-id'])||[]; const matching=allRows.filter(matchesRequest); return (showClosed.checked||!isClosed(s)||matching.length>0)&&(showEmpty.checked||matching.length>0)&&matchesState(s,allRows)}).sort(byNewest); const activeGroups=[]; const otherGroups=[]; sessions.forEach(s=>{const allRows=grouped.get(s['session-id'])||[]; const matching=allRows.filter(matchesRequest); if(matching.some(r=>ACTIVE_REQUEST_STATES.has(r.state))) activeGroups.push([s,matching,allRows]); else otherGroups.push([s,matching,allRows]);}); const oneShots=requests.filter(r=>(!r['session-id']||!knownSessionIds.has(r['session-id']))&&matchesRequest(r)).sort(byNewest); const activeOneShots=oneShots.filter(r=>ACTIVE_REQUEST_STATES.has(r.state)); const otherOneShots=oneShots.filter(r=>!ACTIVE_REQUEST_STATES.has(r.state)); if(!activeGroups.length&&!otherGroups.length&&!oneShots.length)return ''; const tally={}; sessions.forEach(s=>{tally[s.state]=(tally[s.state]||0)+1}); const pills=Object.entries(tally).map(([state,n])=>`<span class="pill ${stateClass(state)}">${n} ${esc(state)}</span>`).join('')+(oneShots.length?`<span class="pill">${oneShots.length} unbound</span>`:''); const section=(title,groups,shotRows)=>{if(!groups.length&&!shotRows.length)return ''; return `<section class="work-section"><h3>${title}</h3><div class="sessions">${groups.map(([s,rows,allRows])=>sessionCard(s,rows,rows.length===0,allRows)).join('')}</div>${shotRows.length?`<div class="orphan"><div class="request-grid">${requestRows(shotRows)}</div></div>`:''}</section>`}; return `<section class="project"><div class="project-head"><div><div class="project-name"><span class="icn">▣</span>${esc(project.name)}</div><div class="queue">${esc(queueSummary(project.queues))}</div></div><div class="pills">${pills}</div></div>${section('Active',activeGroups,activeOneShots)}${section('Other',otherGroups,otherOneShots)}</section>`; }
