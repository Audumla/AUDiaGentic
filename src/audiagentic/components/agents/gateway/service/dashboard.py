...
function requestRows(rows) { return rows.sort(byNewest).map(r=>`<tr><td><code>${esc(r['request-id'])}</code></td><td>${badge(r.state)}${r['provider-turn-pending']?' <span class="live">unresolved</span>':''}${watchdogFlag(r)}</td><td>${esc(r['execution-profile-id']||'')} / ${esc(r['resolved-provider-id']||'')}</td><td>${stamp(recent(r))}</td><td class="error">${esc(r.error?.code||'')} ${esc(r.error?.message||'')}</td></tr>`).join('')||'<tr><td colspan="5" class="muted">No matching requests</td></tr>'; }
