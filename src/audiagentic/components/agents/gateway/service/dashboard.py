...
function requestRows(rows) { return rows.sort(byNewest).map(r=>{const d=r.diagnostics||{}; return `<tr><td><code>${esc(r['request-id'])}</code></td><td>${badge(r.state)}${r['provider-turn-pending']?' <span class="live">unresolved</span>':''}${watchdogFlag(r)}</td><td>${activityLabel(r)}</td><td>${esc(r['execution-profile-id']||'')} / ${esc(r['resolved-provider-id']||'')}</td><td>${stamp(recent(r))}</td><td class="error">${esc(d.classification||r.error?.code||'')} ${esc(d['recovery']?.disposition||r.error?.message||'')}</td></tr>`}).join('')||'<tr><td colspan="6" class="muted">No matching requests</td></tr>'; }
