...
def _subject_resolves(descriptor: ProviderDescriptor, fact: ProviderCapabilityFact) -> bool:
    ...
    if subject.startswith("external:"):
        return bool(subject.removeprefix("external:").strip())
...
def validate_provider_capability_facts(descriptor: ProviderDescriptor) -> None:
    ...
    for fact in descriptor.capability_facts:
        ...
        # Evidence is a plain provenance trail, not a review process: no
        # evidence-tier or review-state workflow validation (slim model, PC04).
        # Subject linkage is still checked so a fact points at something real.
