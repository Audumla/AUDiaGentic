...
</style></head><body><main class='shell'><header class='top'><div><div class='eyebrow'>AUDiaGentic · shared gateway</div><div class='title'>Agent operations</div><div class='sub'>Projects, sessions, queue and provider activity</div></div><div class='pulse'><span class='dot'></span><span id='health'>Starting</span></div></header>
...
<script>window.renderDashboard=function(v){v=v||{};const ss=[...(v.sessions||[]),...(v.jobs||[])],q=v.queue||{},providers=v.providers||[];const state=String(v.runtime||v.state||'unknown');document.title='Agent gateway · '+state+' · '+ss.length+' jobs';document.getElementById('runtime').textContent=state;document.getElementById('health').textContent=['available','ready','healthy','operational'].includes(state.toLowerCase())?'Operational':state;document.getElementById('sessions').textContent=(v.active_sessions??ss.filter(s=>!['closed','completed','failed','cancelled'].includes(statusOf(s))).length);document.getElementById('running').textContent=q.running??ss.filter(s=>['busy','generating','running','executing','in_progress'].includes(statusOf(s))).length;document.getElementById('queued').textContent=q.queued??0;document.getElementById('count').textContent=ss.length+' total'+(providers.length?' · '+providers.length+' providers':'');document.getElementById('updated').textContent=new Date().toLocaleTimeString();const root=document.getElementById('jobs');root.innerHTML=ss.length?ss.map(s=>{const st=statusOf(s),cls=classFor(st),flags=flagsOf(s),provider=s.provider||s.provider_id||'';return `<article class='job'><i class='bar ${cls}'></i><div><div class='jobtitle'>${esc(s.project||s.title||s.name||provider||'Agent job')}</div><div class='meta'>${esc(provider)}${s.session||s.session_id?' · '+esc(s.session||s.session_id):''}${s.page?' · page '+esc(s.page):''}${s.turn?' · turn '+esc(s.turn):''}</div>${flags.length?`<div class='meta'>${esc(flags.join(', '))}</div>`:''}</div><span class='badge'>${esc(st)}</span></article>`}).join(''):'<div class="empty">No active gateway jobs or sessions</div>'};function statusOf(s){const o=s.observed||{};return String(s.status||s.state||s.lifecycle||o.status||o.state||'unknown')}function flagsOf(s){const f=s.flags??s.observed?.flags??s.observed?.markers??[];return Array.isArray(f)?f:Object.entries(f||{}).filter(([,v])=>v).map(([k])=>k)}function classFor(s){s=s.toLowerCase();return /fail|error|crash|reject|cancel/.test(s)?'failed':/ready|complete|closed|idle|success/.test(s)?'complete':'busy'}function esc(x){return String(x).replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]))}</script></body></html>"""
