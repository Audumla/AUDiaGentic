...
class CdpClient:
    ...
    def __init__(self, endpoint: str, *, default_timeout: float = 30.0) -> None:
    ...
    async def start(self) -> None:
        if self._socket is not None:
            return
        websocket_url = await asyncio.to_thread(self._discover_websocket_url)
        self._socket = await connect(websocket_url, max_size=None)
    ...
    def _discover_websocket_url(self) -> str:
        ...
        with urllib.request.urlopen(url, timeout=self.default_timeout) as response:
    ...
    async def _read_loop(self) -> None:
        ...
        try:
            ...
            for future in tuple(self._pending.values()):
                if not future.done():
                    future.set_exception(CdpError(f"CDP connection closed: {exc}"))
            await self.events.put(CdpEvent("cdp.disconnected", {"error": str(exc)}))
