...
def _sync_managed_entries(
    ...
    *,
    ...
    When ``managed_ids`` is ``None``, all managed entries for the provider are
    synced.  When a non-empty set is passed, only entries whose managed id is
    in the set are touched.
    """
    descriptor = _descriptor(provider_id)
    spec = descriptor.mcp_config
    if spec is None:
    ...
    removal_source = (
        {mid: name for mid, name in provider_registry.items() if mid in managed_ids}
        if managed_ids is not None
        else dict(provider_registry)
    )
