...
def build_server() -> FastMCP:
    ...
    def install_component(component_id: str) -> dict[str, Any]:
        ...
        if result.get("ok", True):
            refreshed = refresh_harness_config_if_installed(project_root, reason="component-installed", component_id=component_id)
    ...
    def uninstall_component(component_id: str, remove_configs: bool = False) -> dict[str, Any]:
        ...
        if result.get("ok", True):
            refreshed = refresh_harness_config_if_installed(project_root, reason="component-uninstalled", component_id=component_id)
            result = {**result, "harness_config_refreshed": refreshed}
        return result
    ...
    def enable_component(component_id: str) -> dict[str, Any]:
        ...
        if result.get("ok", True):
            refreshed = refresh_harness_config_if_installed(project_root, reason="component-enabled", component_id=component_id)
            result = {**result, "harness_config_refreshed": refreshed}
        return result
    ...
    def disable_component(component_id: str) -> dict[str, Any]:
        ...
        if result.get("ok", True):
            refreshed = refresh_harness_config_if_installed(project_root, reason="component-disabled", component_id=component_id)
            result = {**result, "harness_config_refreshed": refreshed}
        return result
