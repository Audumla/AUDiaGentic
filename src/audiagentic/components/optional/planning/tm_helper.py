...
def _find_project_root() -> Path:
    ...
    # Strategy 3: Last resort — src/audiagentic/components/optional/planning/ → 5 levels up
    likely_root = module_dir.parents[4]
...
def list_kind(
    ...
    Args:
        kind: Optional kind filter. Can be a single kind or list of kinds.
        root: Optional project root. If None, uses current root.
        ...
        state: Filter by state. Can be a single state or list of states.
...
def check_sensitive_data(id_: str, root: Path | None = None) -> dict[str, Any]:
    ...
    Returns:
        dict with id, has_sensitive_data, warnings, patterns_checked
    """
    if root:
        set_root(root)
