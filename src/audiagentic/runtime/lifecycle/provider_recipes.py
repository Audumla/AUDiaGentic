...
def _enabled_provider_ids(root: Path) -> list[str]:
    """Generic query for enabled provider IDs — no capability specifics."""
    from audiagentic.components.providers.services.provider_config import (
        resolve_provider_enabled,
    )

    return [pid for pid in _all_provider_ids(root) if resolve_provider_enabled(root, pid)]


def _all_provider_ids(root: Path) -> list[str]:
    """Every registered provider ID — used to prune stale integration from disabled ones."""
