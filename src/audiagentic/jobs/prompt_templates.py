...
def _template_roots(project_root: Path, tag: str, provider_id: str) -> list[Path]:
    root = project_root / ".audiagentic" / "prompts" / tag
    return [
...
def load_prompt_template(project_root: Path, *, tag: str, provider_id: str, template_name: str | None = None) -> tuple[str | None, Path | None]:
    candidates = []
    if template_name:
        candidates.append(project_root / ".audiagentic" / "prompts" / tag / f"{template_name}.md")
        candidates.append(project_root / ".audiagentic" / "prompts" / tag / provider_id / f"{template_name}.md")
