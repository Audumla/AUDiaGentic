...
def bootstrap_project(
    ...
    *,
    ...
    files: dict[Path, str] = {
        ...
        root / ".audiagentic" / "knowledge" / "profiles" / "selected.yml": dump_yaml(
            {"capability_profile": capability_profile, "host_profile": host_profile}
        ),
...
def _base_config(knowledge_root: str, config_relpath: str) -> dict[str, Any]:
    return {
        ...
        "search": {
            "snippet_length": 220,
            "weights": {"title": 4.0, "tags": 3.0, "sections": 2.0, "body": 1.0},
        },
        "scaffolding": {
            "default_sections": [
                "Summary",
    ...
    return {
        "event_actions": {
            "generate_sync_proposal": {
                "handler": "audiagentic_knowledge.events:action_generate_sync_proposal"
            },
            "mark_stale": {"handler": "audiagentic_knowledge.events:action_mark_stale"},
            "mark_stale_and_generate_sync_proposal": {
                "handler": "audiagentic_knowledge.events:action_mark_stale_and_generate_sync_proposal"
            },
        ...
        "deterministic_actions": {
            ...
            "generate_sync_proposals": {
                "handler": "audiagentic_knowledge.actions:action_generate_sync_proposals"
            },
            "cleanup_lifecycle": {
                "handler": "audiagentic_knowledge.actions:action_cleanup_lifecycle"
            },
            ...
            "record_event_baseline": {
                "handler": "audiagentic_knowledge.actions:action_record_event_baseline"
            },
            "seed_from_manifest": {
                "handler": "audiagentic_knowledge.actions:action_seed_from_manifest"
            },
...
def _default_llm() -> dict[str, Any]:
    return {
        "providers": {
            ...
            "mock_overseer": {
                "handler": "audiagentic_knowledge.llm:provider_mock_overseer",
                "defaults": {"supports_async": True, "supports_blocking": True},
            },
            "openai_compatible_remote": {
                "handler": "audiagentic_knowledge.llm:provider_disabled",
                "defaults": {
                    "note": "Placeholder registry entry for a future remote provider adapter."
                },
            },
        },
        "profiles": {
            "knowledge-triage": {
                "description": "Small fast profile for classification and source triage.",
                "capabilities": ["classification", "reranking", "short_summary"],
                "supports_blocking": True,
                "supports_async": True,
                "structured_output": True,
                "citation_friendly": True,
            },
            "knowledge-proposal": {
                "description": "Structured proposal drafting profile.",
                "capabilities": ["proposal_synthesis", "diff_summary"],
                "supports_blocking": True,
                "supports_async": True,
                "structured_output": True,
                "citation_friendly": True,
            },
            "knowledge-deep-answer": {
                "description": "Broader synthesis profile for answering questions over retrieved pages.",
                "capabilities": ["question_answering", "topic_brief"],
                "supports_blocking": True,
                "supports_async": True,
                "structured_output": True,
                "citation_friendly": True,
            },
            "knowledge-bootstrap-scan": {
                "description": "Asynchronous interrogation profile for larger project scans.",
                "capabilities": ["repo_interrogation", "entity_extraction", "relation_extraction"],
...
def _default_navigation() -> dict[str, Any]:
    return {
        "routes": [
            {
                "id": "registry_and_current_state",
                "label": "Current operational state",
                "keywords": ["registry", "build status", "current state", "claim", "verified"],
                "page_ids": [],
                "deterministic_actions": ["scan_drift", "generate_sync_proposals"],
                "note": "Use this route for current operational docs and sync review.",
            },
            {
                "id": "runtime_events",
                "label": "Runtime event artifacts",
                "keywords": ["runtime", "events", "stream", "ndjson"],
                "page_ids": [],
                "deterministic_actions": ["scan_events", "process_events"],
                "note": "Use this route when runtime artifacts may have changed.",
            },
        ],
        "fallbacks": [
            {
                "action_id": "search_pages",
                "keywords": ["find", "where", "what page"],
                "label": "Search pages",
                "note": "Lexical search across current-state pages.",
            },
            {
                "action_id": "scan_drift",
                "keywords": ["drift", "stale", "sync"],
                "label": "Scan drift",
                "note": "Deterministic fallback when sources may have changed.",
            },
            {
                "action_id": "scan_events",
                "keywords": ["event", "runtime"],
                "label": "Scan events",
                "note": "Deterministic fallback for runtime/planning event artifacts.",
            },
            {
                "action_id": "doctor",
                "keywords": ["doctor", "health", "install"],
                "label": "Doctor",
                "note": "Check the capability contract and project-owned files.",
            },
        ],
    }
...
Project-owned:
- the vault under `{knowledge_root}/`, sidecars, state, manifests, event adapters, and proposals.
...
- Registries, navigation, manifests, and event adapters are all YAML-driven.
...
2. Edit event adapters under `{knowledge_root}/events/adapters.yml`.
