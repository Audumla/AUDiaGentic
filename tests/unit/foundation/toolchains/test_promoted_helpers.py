...
class TestSubstituteParams:
    def test_replaces_known_placeholders(self) -> None:
        out = substitute_params("curl {URL}/api --token {TOKEN}", {"URL": "http://x", "TOKEN": ""})
        assert out == "curl http://x/api --token "

    def test_leaves_unknown_placeholders_literal(self) -> None:
        assert substitute_params("run {MYSTERY}", {"URL": "x"}) == "run {MYSTERY}"

    def test_empty_text_passthrough(self) -> None:
        assert substitute_params("", {"URL": "x"}) == ""
...
class TestStepsFromDefs:
    ...
    def test_unknown_type_raises(self) -> None:
        with pytest.raises(AudiaGenticError, match="VAL-PSTEP-001"):
            steps_from_defs([{"type": "nope"}], {})
