...
an existing-but-empty provider-policy.yaml (the installer's comment-only marker)
...
_PATH_PARTS = (".audiagentic", "config", "provider-policy.yaml")


def _policy_yaml_path(root):
    path = root
...
def test_set_allowlist_round_trips(tmp_path):
    ...
    on_disk = load_yaml_file(_policy_yaml_path(tmp_path))
...
def test_set_invalid_mode_raises_and_does_not_write(tmp_path):
    ...
    assert not _policy_yaml_path(tmp_path).exists()
...
def test_schema_rejects_invalid_mode_directly(tmp_path):
    ...
    path = _policy_yaml_path(tmp_path)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
