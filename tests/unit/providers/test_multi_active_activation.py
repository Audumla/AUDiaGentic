...
def test_enabling_one_provider_does_not_deselect_another(tmp_path: Path) -> None:
    ...
    assert get_implementation_state(tmp_path, "providers", first).enabled is True
    assert get_implementation_state(tmp_path, "providers", second).enabled is True
...
def test_disabling_one_provider_leaves_others_active(tmp_path: Path) -> None:
    ...
    assert get_implementation_state(tmp_path, "providers", first).enabled is False
    assert get_implementation_state(tmp_path, "providers", second).enabled is True
