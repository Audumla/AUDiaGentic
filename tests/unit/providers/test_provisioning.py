...
def test_cli_install_does_not_dispatch_other_automation_families(
    ...
    monkeypatch.setattr(lifecycle, "_seed_provider_config", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        lifecycle,
        "_build_surface_request",
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("CLI lifecycle must not dispatch generated surfaces")
        ),
    )
    monkeypatch.setattr(
        lifecycle,
        "_sync_provider_mcp",
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("CLI lifecycle must not dispatch managed MCP")
        ),
    )

    result = lifecycle.install_provider_cli("codex", project_root=tmp_path)
...
def test_cli_lifecycle_plan_predicts_install_when_cli_absent(
    ...
    assert result.action_needed == "install"
...
def test_cli_lifecycle_result_rejects_invalid_state_with_owned_error() -> None:
    import pytest
    ...
    with pytest.raises(AudiaGenticError) as exc:
        CliLifecycleResult(ok=True, supported=True, state="unknown")

    assert exc.value.code == "VAL-PCLI-001"
