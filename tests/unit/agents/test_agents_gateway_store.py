...
def test_provider_activity_retains_signal_and_semantic_group(tmp_path: Path) -> None:
    record = store.build_record(execution_profile_id="default", prompt_body="hello")
    store.write_record(tmp_path, record)
    claimed = store.claim_dispatch(tmp_path, record["request-id"], owner_epoch="service-a", expected_revision=0)
    running = store.start_owned_attempt(
    ...
    provider = renewed["activity"]["provider"]
    assert provider["phase"] == "talked-to-app"
    assert provider["activity-label"] == "talked-to-app"
    assert provider["activity-group"] == "external-app"
    evidence = renewed["diagnostic-evidence"][-1]
    assert evidence["activity-label"] == "talked-to-app"
    assert evidence["activity-group"] == "external-app"
