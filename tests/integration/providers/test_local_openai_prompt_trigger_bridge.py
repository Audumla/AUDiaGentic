...
def test_local_openai_prompt_trigger_bridge_script_launches_job(tmp_path: Path) -> None:
    sandbox = sandbox_helper.create(tmp_path, "local-openai-prompt-trigger-bridge")
    try:
        _write_project_and_provider_config(sandbox)
        result = subprocess.run(
            [
                sys.executable,
                str(ROOT / "src" / "audiagentic" / "components" / "prompt_triggers" / "local_openai_prompt_trigger_bridge.py"),
                "--project-root",
                str(sandbox.repo),
            ],
            cwd=ROOT,
            input="@plan target=packet:PKT-JOB-008 provider=local-openai\n"
            "Plan the next stage.\n",
            capture_output=True,
            text=True,
            check=False,
