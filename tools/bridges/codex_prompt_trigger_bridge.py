"""Codex-specific prompt-trigger bridge wrapper."""
...
def run(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="codex-prompt-trigger-bridge")
            ...
            json.dumps(
                {
                    "status": "error",
                    "kind": "validation",
                    "message": "missing Codex prompt-calling assets",
                    "missing": missing,
...
if __name__ == "__main__":
    raise SystemExit(run(["--provider-id", "codex", *sys.argv[1:]]))
