...
This package freezes the fixture catalog, regression matrix, migration checks, and modular packetization outputs for later implementation intake. It consumes outputs from `wp-28`, `wp-29`, and `wp-30` plus the external import-readiness map and validation report. It does not perform live planning import or direct repo code changes.

# Inputs

- request-32
- spec-84
- outputs from `wp-28`, `wp-29`, and `wp-30`
...
1. Freeze fixture catalog (task-356).
2. Freeze regression matrix and evidence expectations (task-357).
3. Freeze modular packet outputs and handoff boundaries (task-358).
...
7. Run a spec consistency check across `spec-81`, `spec-82`, `spec-83`, and `spec-84` before freezing packetization outputs.
