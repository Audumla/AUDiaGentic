...
- spec-85 present with manifest schema, disable semantics, and uninstall semantics
- task-359 present in wp-28 with knowledge component as reference case
- task-360 present in wp-29 with stage-one apply boundaries table
- `--disable-component` flag behavior is defined in task-360, not just declared in spec-80
- replacement slot pattern (`replaces` field) is documented
